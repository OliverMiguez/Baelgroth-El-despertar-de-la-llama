...
var _ultima_pos_goblin_visible: Vector2 = Vector2.ZERO  
...
func tick(delta: float):
	...
	if objetivo is goblin_principal:
		if not objetivo.escondido:
			# Goblin visible, actualizamos su ultima posicion conocida
			_ultima_pos_goblin_visible = objetivo.global_position  # AÑADIDO
			_perseguir_posicion(_ultima_pos_goblin_visible, delta)
		else:
			if enemigo.goblin_fue_visto:
				# Escondido pero visto antes, vamos a donde estaba, no donde esta
				_perseguir_posicion(_ultima_pos_goblin_visible, delta)  # MODIFICADO
				# Si llegamos a ese punto, pasamos a investigar
				if agente.is_navigation_finished():
					_iniciar_pausa_perdida(_ultima_pos_goblin_visible)
			else:
				_iniciar_pausa_perdida(objetivo.global_position)
			return

	_perseguir_posicion(objetivo.global_position, delta)
...
func _perseguir_posicion(posicion: Vector2, delta: float):
...
# Mantenemos _perseguir como wrapper para no romper nada
func _perseguir(objetivo: Node2D, delta: float):
	_perseguir_posicion(objetivo.global_position, delta)
